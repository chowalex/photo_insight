...
AnalyzeSelectedPhotos.lua
Uses GPT-4o to analyze an image and populate an image's title and
...
local logger = LrLogger('AnalyzeSelectedPhotos')
...
local function setTitle(catalog, photo, title)  
...
local function describeImage(catalog, photo, base64Image, gps)
  local headers = {
  ...
  local body = {
    ...
    messages = {
      {
        role = "system", 
        content = "You are an assistant that generates titles and keywords for photos for the user. Given a photograph, provide a short, succinct title for the provided photograph, identifying any landmarks and points of interest, if applicable. If location coordinates are provided, use the location information to help with identifying potential landmarks. Avoid overly general descriptions like 'a building under a clear blue sky' or 'people walking on the street'. The output should be a JSON object with a 'title' field and a 'keywords' field, which is a list of strings."
      },
      {
            role = "user",
            content = {
  ...
  if result then
      ...
      if response and response.choices and response.choices[1] and response.choices[1].message and response.choices[1].message.content then
          ...
          -- The API sometimes wraps content in ```json...``` and sometimes doesn't.
          local jsonContent = content:match("```json(.-)```") or content
          local parsedContent = JSON:decode(jsonContent)

          if parsedContent and parsedContent.title and parsedContent.keywords then
            setTitle(catalog, photo, parsedContent.title)            
          else
...
local function exportAndDescribeImage()
  ...
  for i, photo in ipairs(photos) do
    ...
    progressScope:setPortionComplete(0.1*i/N, 1)  

    for _, rendition in exportSession:renditions() do
        local success, pathOrMessage = rendition:waitForRender()
        progressScope:setPortionComplete(0.3*i/N, 1)
        ...
        if success then
            ...
            progressScope:setPortionComplete(0.4*i/N, 1)

            describeImage(catalog, photo, base64Image, gps)
            progressScope:setPortionComplete(0.9*i/N, 1)
